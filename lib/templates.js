...
export function generateArticlePage(post, config) {
  ...
  const isWikimedia = hasHero && (hero.sourceUrl || '').includes('wikimedia.org');
  const imageCredit = hasHero ? `
      <div class="image-credit">
        ...
        授權：<a href="${escHtml(hero.licenseUrl)}" target="_blank" rel="noopener">${escHtml(hero.license)}</a>${isWikimedia ? `
  ...
  <header class="site-header">
    <div class="container">
      ...
      <nav class="header-tabs" id="categoryTabs">
        ...
        <button class="tab-btn" data-cat="巴柔">巴柔</button>
